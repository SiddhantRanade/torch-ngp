...
def run_colmap(args):
    ...
    if os.path.exists(db):
        os.remove(db)
    do_system(
        f"colmap feature_extractor --ImageReader.camera_model OPENCV --SiftExtraction.estimate_affine_shape=true --SiftExtraction.domain_size_pooling=true --ImageReader.single_camera 1 --SiftExtraction.first_octave 0 --database_path {db} --image_path {images}"
    )
    do_system(
